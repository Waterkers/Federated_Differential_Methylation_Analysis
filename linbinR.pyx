#!python
#cython: boundscheck=False, wraparound=False, cdivision=True
"""
cython -a fast_linbin.pyx
gcc -shared -pthread -fPIC -fwrapv -O2 -Wall -fno-strict-aliasing -I/usr/include/python2.7 -I/usr/local/lib/python2.7/dist-packages/numpy/core/include/ -o fast_linbin.so fast_linbin.c
"""

cimport cython
cimport numpy as np
import numpy as np

ctypedef np.float64_t DOUBLE
ctypedef np.int_t INT
ctypedef np.int64_t LONG

def fast_linbin(np.ndarray[DOUBLE] X, double a, double b, int M, np.ndarray[DOUBLE] w):#, int trunc=1):
    """
    Linear Binning as described in Fan and Marron (1994)
    """
    cdef:
        Py_ssize_t i, li_i
        int nobs = np.PyArray_DIMS(X)[0]
        double delta = (b - a)/(M - 1)
        np.ndarray[DOUBLE] gcnts = np.zeros((2*M), float)
        np.ndarray[DOUBLE] lxi = (X - a)/delta
        np.ndarray[INT] li = np.floor(lxi).astype(int)
        np.ndarray[DOUBLE] rem = lxi - li
        int m_max = M-2

    for i in range(nobs):
        li_i = li[i]
        if li_i > 0 and li_i < M:
            gcnts[li_i] = gcnts[li_i] + ((1 - rem[i]) * w[i])
            gcnts[li_i+1] = gcnts[li_i+1] + (rem[i] * w[i])
        if li_i == m_max+1:
            gcnts[li_i] = gcnts[li_i] + ((1 - rem[i]) * w[i])
        if li_i == -1:# and trunc == 0:
            gcnts[0] = gcnts[0] + (rem[i] * w[i])
    return gcnts