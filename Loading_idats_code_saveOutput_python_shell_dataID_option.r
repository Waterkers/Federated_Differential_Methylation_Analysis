...
# function to create the output directory - with or without data identifier in folder names
create_output_file_structure <- function(output_dir, identifier = NULL) {
setwd(output_dir)
if (!is.null(identifier)) {
	if(!dir.exists(sprintf("QC_%s_PythonShell", identifier))){
	dir.create(sprintf("QC_%s_PythonShell", identifier))
}
if(!dir.exists(sprintf("QC_%s_PythonShell\\Polts", identifier))){
  dir.create(sprintf("QC_%s_PythonShell\\Polts", identifier))
}
} else {
if(!dir.exists("QC_PythonShell")){
  dir.create("QC_PythonShell")
}
if(!dir.exists("QC_PythonShell\\Plots")){
  dir.create("QC_PythonShell\\Plots")
}
}
...
if (exists(input[4])) {
	data_id <- input[4]
	create_output_file_structure(working_dir, data_id)
} else {create_output_file_structure(working_dir)}

load_and_save(idat, pheno_info)
