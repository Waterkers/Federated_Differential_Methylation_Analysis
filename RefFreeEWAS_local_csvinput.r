...
# load in the methylSet object
	...
	betas <- read.csv(msetEPIC.pf, header = TRUE, row.names = 1)
	pheno <- read.csv(QCmetrics, header = TRUE)
	annotation_data <- read.csv(manifest_path , skip = 7, header = TRUE)
	retained_annotation <- annotation_data[annotation_data$IlmnID %in% rownames(betas), ]
...
# because they can interfere with the cell-type decomposition.
	temp_data = betas[retained_annotation$CHR!="X", ] #something funky going on here
